...
export function BudgetAnalyzer({ expenses, budgets }: BudgetAnalyzerProps) {
  ...
  // Initialize ApexCharts when analysis data is available
  useEffect(() => {
    if (!analysis) return;

    // Area Chart - Monthly Spending Trend
    if (areaChartRef.current && analysis.monthlyTrend?.length > 0) {
      const areaChart = new ApexCharts(areaChartRef.current, {
        ...
        series: [
          {
            name: 'Budget',
            data: analysis.monthlyTrend.map(item => item.budget),
          },
          {
            name: 'Spent',
            data: analysis.monthlyTrend.map(item => item.spent),
          },
        ],
        xaxis: {
          categories: analysis.monthlyTrend.map(item => item.month),
          labels: { style: { colors: 'rgb(107 114 128)', fontSize: '12px' } },
        },
        yaxis: {
          labels: {
        ...
        grid: { borderColor: 'rgb(229 231 235)', strokeDashArray: 3 },
        tooltip: {
          theme: 'light',
  ...
  useEffect(() => {
    ...
    if (pieChartRef.current && analysis.topCategories?.length > 0) {
      const pieChart = new ApexCharts(pieChartRef.current, {
        ...
        series: analysis.topCategories.map(item => item.amount),
        labels: analysis.topCategories.map(item => item.category),
        colors: COLORS,
        legend: {
          position: 'bottom',
          fontSize: '12px',
  ...
  useEffect(() => {
    ...
    // Bar Chart 1 - Weekly Spending Pattern
    if (barChart1Ref.current && analysis.spendingPattern?.length > 0) {
      const barChart1 = new ApexCharts(barChart1Ref.current, {
        chart: {
          type: 'bar',
          height: 300,
          fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif',
        ...
        series: [{
          name: 'Average Spent',
          data: analysis.spendingPattern.map(item => item.amount),
        }],
        xaxis: {
          categories: analysis.spendingPattern.map(item => item.day),
          labels: { style: { colors: 'rgb(107 114 128)', fontSize: '12px' } },
        },
        yaxis: {
          labels: {
  ...
  useEffect(() => {
    ...
    // Bar Chart 2 - Budget vs Actual
    if (barChart2Ref.current && analysis.categoryAnalysis?.length > 0) {
      const budgetData = analysis.categoryAnalysis.map(cat => ({
        category: cat.category,
        budget: 100,
        spent: cat.percentage,
      }));

      const barChart2 = new ApexCharts(barChart2Ref.current, {
        chart: {
          type: 'bar',
          height: 300,
          fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif',
        ...
        xaxis: {
          categories: budgetData.map(item => item.category),
          labels: { style: { colors: 'rgb(107 114 128)', fontSize: '12px' } },
        },
        yaxis: {
          labels: {
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <div ref={areaChartRef} className="w-full h-[300px]"></div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div ref={barChart1Ref} className="w-full h-[300px]"></div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div ref={barChart2Ref} className="w-full h-[300px]"></div>
              </CardContent>
            </Card>
          </div>
