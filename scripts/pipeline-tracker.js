...
const runDeployment = async () => {
    ...
    if (!webappName || !resourceGroup) {
        log.warning('Azure deployment skipped - missing AZURE_WEBAPP_NAME or AZURE_RESOURCE_GROUP');
        return {
            status: 'skipped',
            duration: 0,
            url: null,
    ...
    try {
        log.deploy(`Starting deployment to ${webappName}...`);
        
        // Create deployment package
        execSync('tar -czf deploy.tar.gz dist/ package.json server.js', { 
            cwd: projectRoot 
        });
        
        // Deploy to Azure
        const deployOutput = execSync(
            `az webapp deployment source config-zip --resource-group ${resourceGroup} --name ${webappName} --src deploy.tar.gz`,
        ...
        log.success(`Deployment completed successfully to ${deployUrl}`);
    } catch (error) {
        deployStatus = 'failed';
        deployError = error.message;
        log.error(`Deployment failed: ${deployError}`);
    }
